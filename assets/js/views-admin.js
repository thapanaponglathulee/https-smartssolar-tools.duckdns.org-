...
(function () {
  ...
  SS.views.settings = function (host) {
    ...
    h += '<div class="card"><div class="chead"><h2>ใบลาและการอนุมัติ</h2><span class="sp"></span>' + U.ref(['BR-09', 'APV-07', 'APV-08', 'LV-09']) + '</div>' +
      ...
      '</div>' +
      f('maxLeavePerDay', 'เพดานคนลาพร้อมกันต่อวัน', 'number', 'เว้นว่าง = ยังไม่บังคับ · <b>ตัวเลขนี้ยังไม่เคาะ</b>') +
      '</div>';
  ...
  SS.views.staff = function (host) {
    var h = U.note('mock', 'เจ้าของจริงของทะเบียนนี้คือเมนู Staff/Account (ของกลาง S1 · S8 · S15 · S16) ซึ่งยังไม่ถูกสร้าง — ตารางนี้เป็นของจำลองเพื่อให้ทดสอบ flow ได้');
  ...
  SS.views.tables = function (host) {
    ...
    var h = '<div class="card"><div class="chead"><h2>S19 · ทะเบียนประเภทงาน</h2><span class="sp"></span>' + U.ref(['CI-15', 'CI-16', 'CI-18']) + '</div>' +
      '<div class="tw"><table class="t"><thead><tr><th>ประเภทงาน</th><th>รหัส</th><th>เลือกไซต์</th><th>ลิสต์ไซต์ที่แสดง</th><th>ลักษณะการไปตั้งต้น</th><th class="n">มื้อ</th><th>สถานะ</th></tr></thead><tbody>' +
    ...
    h += '<div class="card"><div class="chead"><h2>S10 · ทะเบียนโครงการ/ไซต์งาน</h2><span class="sub">เปลี่ยนสถานะแล้วลิสต์ในหน้าเช็คอินเปลี่ยนทันที</span><span class="sp"></span>' + U.ref(['CI-01', 'CI-16', 'ALW-02']) + '</div>' +
      '<div class="tw"><table class="t"><thead><tr><th>รหัส</th><th>ชื่อโครงการ</th><th>สถานะ</th><th>อัตราเบี้ยเลี้ยง (บาท/มื้อ)</th></tr></thead><tbody>' +
      db.projects.map(function (p) {
        ...
        return '<tr><td>' + esc(p.code) + '</td><td>' + esc(p.name) + '</td>' +
          ...
          '<td>' + r.rate + (r.isDefault ? ' ' + U.pill('pill-orange', 'ใช้อัตราเริ่มต้นของบริษัท') : '') + '</td></tr>'; }).join('') +
      '</tbody></table></div>' +
  ...
  var MAP = [
    ...
    ['CI-05', 'ระบบระบุประเภทวันให้เอง', 'หน้าเช็คอิน — ไม่มีดรอปดาวน์ประเภทวัน', 'done', ''],
    ...
    ['CI-10', 'บันทึกการทำงานนอกเวลา', 'หน้าเช็คอิน → ปุ่มบันทึกการทำงานนอกเวลา', 'done', ''],
    ...
    ['CI-14', 'สถานะรายวันของพนักงาน', 'ทีมของฉัน · Dashboard · ปฏิทิน', 'done', 'คำนวณตอนอ่านทุกครั้ง'],
    ...
    ['CI-18', 'เติมลักษณะการไปให้อัตโนมัติ แก้ได้พร้อมเหตุผล', 'หน้าเช็คอิน', 'done', ''],
    ['CI-19', 'ปรับรายการลักษณะการไปงาน', 'ทะเบียนของกลาง → S20', 'done', ''],
  ...
  SS.views.specmap = function (host) {
    var badge = { done: ['pill-green', 'ทดสอบได้'], partial: ['pill-yellow', 'ทดสอบได้บางส่วน'],
                  cant: ['pill-red', 'ทดสอบที่นี่ไม่ได้'], dropped: ['pill-gray', 'ยกเลิกแล้ว'] };
    var n = { done: 0, partial: 0, cant: 0, dropped: 0 };
    ...
    var h = '<div class="grid g4">' +
      ...
      '<div class="stat k"><div class="l">ยกเลิกแล้ว</div><div class="v">' + n.dropped + '</div></div></div>';
